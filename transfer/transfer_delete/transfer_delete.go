...
import (
	...
	"axia4/schema/collection"
	"axia4/schema/column"
	"axia4/schema/doc"
	"axia4/schema/field"
	"axia4/schema/form"
	"axia4/schema/icon"
	"axia4/schema/jsFunction"
	"axia4/schema/loginForm"
	"axia4/schema/menuTab"
...
func NotExisting_tx(ctx context.Context, tx pgx.Tx, module types.Module) error {
	...
	// search bars
	if err := deleteSearchBars_tx(ctx, tx, module.Id, module.SearchBars); err != nil {
		return err
	}
...
func deleteSearchBars_tx(ctx context.Context, tx pgx.Tx, moduleId uuid.UUID, bars []types.SearchBar) error {
	idsKeep := make([]uuid.UUID, 0)
